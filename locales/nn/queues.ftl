...
queue-idle = lediggjengde
queue-running = køyrer
queue-queued = køa
agents-available = {$available}/{$total} tilgjengelege

# Kødetaljar
jobs-in-build = Jobbar i dette bygget:
...
# Kødetaljar-visning
queue-summary = 📋 {$queue} kø ({$builds} bygg, {$jobs} jobbar ventar)
